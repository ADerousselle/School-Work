...
def main():
    ...
    #env_values = dotenv_values("../data/input/.env")                #DEBUG:Create a dictionary using the data in a .env file on the dev's computer
    ...
    #json_path = "../data/output/"                                   #DEBUG: Path to the location all of the JSON output files will be placed on dev's computer
    ...
    for project in projects:                                        #For each project
        ...
        for url in url_list:                                                #For each url
            ...
            #print(raw_dict != {})                                           #DEBUG: Check if a non-empty dictionary was returned       
            if raw_dict == {}:
                continue
...
def url_creator(email_config, searchTerms, project):
    ...
    #line_count = 0                                              #DEBUG: Skip the Category terms that will take hours to scrape
    ...
    with open(searchTerms, 'r') as file:                        #Open and read from the searchTerms.txt file
        for term in file:                                           #For each line in the file    
            ...
            #line_count += 1                                             #DEBUG: Skip the Category terms that will take hours to scrape
            #if line_count <= 3:                                         #DEBUG: Skip the Category terms that will take hours to scrape
           #     continue                                                #DEBUG: Skip the Category terms that will take hours to scrape
            ...
            #print(url)                                                  #DEBUG: View the URL created
            ...
            if is_valid_url(url):                                       #If url is valid
                ...
                #debug_send_error(1, url)                                  #DEBUG: Print to screen a message instead of emailing
...
def run_proj(url, err_count, ph_config, em_config, proj_token):
    ...
    while True:
        ...
        #print(run_status)                                                                           #DEBUG: Watch the run status over time
        if (run_status == 'complete' or run_status == 'error'):                                     #If the status is 'complete' or 'error'
            if (run_status == 'error'):                                                                 #If the status is 'error'
                if err_count == 1:                                                                          #If the error count is 1
                    ...
                    #debug_send_error(2, url)                                                                    #DEBUG: Print to screen a message instead of emailing
...
def check_values(dict, config):
    ...
    for prod in dict["Product"]:                                #For each product in the dictionary
        ...
        if prod["Price"] != "":                                     #If the product Price is filled
        ...
        if "Img_URL" not in prod:                                   #If the Img_URL is not in the product dictionary
            prod["Img_URL"] = ""
        if prod["Img_URL"] == "":                                   #If the Img_URL is does not have a value
            prod["Img_URL"] = "NoImage.jpg"                             #Give it the "No Image Available" image as the value
        elif prod["Img_URL"] != "":                                 #If the product Img_URL is filled
    ...
    if name_flag == 1:                                          #If the name flag still indicates that names are not being scraped
        send_error(3, "Name", config)                              #Call send_error with a code of 3 and the field "Name"
        #debug_send_error(3, "Name")                                 #DEBUG: Print to screen a message instead of emailing
    if price_flag == 1:                                         #If the price flag still indicates that prices are not being scraped
        ...
        #debug_send_error(3, "Price")                                #DEBUG: Print to screen a message instead of emailing
    if img_flag == 1:                                           #If the img flag still indicates that image URLs are not being scraped
        ...
        #debug_send_error(3, "Img_URL")                              #DEBUG: Print to screen a message instead of emailing
