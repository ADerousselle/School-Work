...
import warnings
# os.environ['PYTHONWARNINGS'] = 'ignore'  # Also suppress DeprecationWarning
...
def setup_module(module):
    module.data_dir = os.path.join(parent_directory, 'data')
    module.output_dir = os.path.join(parent_directory, 'output')


def test_train_face_model():
    ############### Load data ###############
    output_dir = os.path.join(parent_directory, 'output')
    faces_file_path = os.path.join(data_dir, 'faces1000.npy')
    nonfaces_file_path = os.path.join(data_dir, 'nonfaces1000.npy')
    ...
    # Check that accuracy is at least .97
    assert test_acc >= .97


def test_cnn_face_search1():
    # Load the image data/vjm.bmp
    img = cv2.imread(os.path.join(data_dir, 'vjm.bmp'), cv2.IMREAD_GRAYSCALE)
    ...
    # Save running time information to a file
    with open(os.path.join(output_dir, 'running_time_vjm.txt'), 'w') as f:
...
def test_cnn_face_search2():
    # Load the image data/faces.bmp
    img = cv2.imread(os.path.join(data_dir, 'faces.bmp'), cv2.IMREAD_GRAYSCALE)
    ...
    # Save running time information to a file
    with open(os.path.join(output_dir, 'running_time_faces.txt'), 'w') as f:
        f.write(str(end_time - start_time))
