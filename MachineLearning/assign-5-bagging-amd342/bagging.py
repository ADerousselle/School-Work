...
class CustomBaggingClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, base_estimator=None, n_estimators=10, random_state=None):
        ...
        self.rng = np.random.default_rng(self.random_state_)
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_estimators_):
            ...
            self.trained_bootstrap_models_.append(estimator)
    ...
    def predict(self, X):
        ...
        n_samples = X.shape[0]
        predictions = np.empty((self.n_estimators_, n_samples), dtype=int)
        for i, model in enumerate(self.trained_bootstrap_models_):
    ...
    def predict_proba(self, X):
        ...
        n_samples = X.shape[0]
        predictions = np.empty((self.n_estimators_, n_samples, self.n_classes), dtype=float)
        for i, model in enumerate(self.trained_bootstrap_models_):
    ...
    def _get_bootstrap_sample(self, X, y):
        ...
        n_samples = X.shape[0]
        b_indices = self.rng.choice(n_samples, size=n_samples, replace=True)
        o_indices = np.setdiff1d(np.arange(n_samples), b_indices)
